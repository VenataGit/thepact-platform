...
async function loadBoard(token, account) {
  ...
  const boards = await mapLimit(tools, 4, async (t) => {
    ...
    return { id: table.id, title: t.title || table.title, projectId, columns };
  });
